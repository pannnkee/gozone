...
type Comment struct {
	...
	CreateTimeStr       string `json:"create_time_str"`                                             // 创建时间
...
func (this *Comment) GetSecondComment(articleId, parentCommentId int64) (data []*Comment, err error) {
	db := conn.GetORMByName("zone")
	db = db.Model(this)
	err = db.Order("create_time desc").Where("article_id=?", articleId).Where("comment_level=2").
		Where("parent_comment_id=?", parentCommentId).Find(&data).Error
	return
}
